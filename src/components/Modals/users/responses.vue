...
<template>
  <div v-if="responsesModal && responsesModal.length > 0" 
    class="responses">
    <div v-for="(response, index) in responsesModal" 
    :key="index" 
    class="response">
      ...
      <div v-if="response.status?.id === 3" 
      class="response__buttons">
        <span class="icon remove_response" 
        @click="editStatus(response.id, 2)">
          <img src="/icons/close.svg">
        </span>
        <span class="icon agree_response" 
